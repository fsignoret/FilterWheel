...
const int SLOW_MOTOR_SPEED = 32;
const int MAX_SEARCH_STEPS = 1000;
const int INITIAL_DELAY = 200;
const int NEXT_MOVE_DELAY = 10;
const int STOP_DELAY = 200;
...
void setup()
{
  ...
  motor.setSpeed(FAST_MOTOR_SPEED);
...
bool MoveFilterWheel(int targetPosition)
{
  ...
  int motorReverseDirection = BACKWARD;
  ...
  // Make sure we are accurately positionned
  return MoveSlowlyToCaptor(motorReverseDirection, _currentPosition);
}
...
/*
 * Moves the wheel in order to reach the next captor
...
bool FindNextCaptor(int motorDirection, int newPosition)
{
  ...
  // Forward otherwise
  if (newPosition == _currentPosition && !_isLost)
  {
...
/*
 * Moves the wheel slowly in order to be accurately positionned on the captor
 * Returns true if the captor was found, false otherwise
 */
bool MoveSlowlyToCaptor(int motorDirection, int newPosition)
{
  delay(STOP_DELAY);
